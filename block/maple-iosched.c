...
maple_dispatch_requests(struct request_queue *q, int force)
{
	...
	if (!rq) {
		...
		if (!state_suspend && mdata->starved >= mdata->writes_starved)
			data_dir = WRITE;
		else if (state_suspend && mdata->starved >= 1)
			data_dir = WRITE;
